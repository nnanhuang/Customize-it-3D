...
def process_single_image(image_path, depth_estimator, normal_estimator=None):
    ...
    rgba_path_ori = os.path.join(out_dir, 'rgba.png')
    rgba_path = os.path.join(rgba_dir, 'rgba.png')
    if os.path.exists(rgba_path_ori):
        if not os.path.exists(rgba_path):
            shutil.move(rgba_path_ori, rgba_path)
        else:
            os.remove(rgba_path)
    ...
    out_normal = os.path.join(normal_dir, 'normal.png')
    out_mask = os.path.join(mask_dir, 'mask.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', default='data/failure_cases/armchair/rgba.png', type=str, nargs='*', help="path to image (png, jpeg, etc.)")
