...
class StableDiffusion(nn.Module):
    def __init__(self, trained_model_path, class_name, device, sd_version='2.0', hf_key=None, step_range=[0.2, 0.6]):
        super().__init__()
        ...
        if trained_model_path is not None:
            print(f'[INFO] loading dreambooth stable diffusion...')
            print(f'[INFO] using '+ trained_model_path)
            model_key = trained_model_path
    ...
    def get_text_embeds(self, prompt, negative_prompt):
        ...
        if self.trained_model_path is not None:
            # prompt with identifier for dreambooth
            new_prompt_list = []
            for p in prompt:
                # p = "rgb photo of " + p
                if self.class_name == None:
                    ...
                    index = p.find(self.class_name)
                    if index != -1:
                        p = p[:index] + 'sks ' + p[index:]
                    else:
                        p = 'sks ' + p
    ...
    def train_step(self, text_embeddings, pred_rgb, ref_rgb=None, noise=None, islarge=False, ref_text=None, clip_model=None, guidance_scale=10, grad_scale = 1):
        # adjust batch size = n_view for mvdream guidance
        ...
        # if not islarge and (t / self.num_train_timesteps) <= 0.4:
            ...
            # w(t), sigma_t^2
            w = (1 - self.alphas[t])
            grad = w * (noise_pred - noise) * w_ * grad_scale
            imgs = None
        ...
        return loss, imgs # don't use dummy loss value
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', default='a green and gold chair with gold trim near beach', type=str)
    parser.add_argument('--negative', default='', type=str)
    parser.add_argument('--workspace', default='chair_prompt2', type=str)
    ...
    parser.add_argument('--class_name', default='chair', type=str, help="the class name used in dreambooth")
    parser.add_argument('--trained_model_path', default='out/dreambooth_sd/chair_prompt2', type=str, help="the path of learned model")
    ...
    sd = StableDiffusion(opt.trained_model_path,opt.class_name,device, opt.sd_version)
